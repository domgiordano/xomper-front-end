...
export class LeagueComponent implements OnInit {
  ...
  getMatchups(): void {
    this.loading = true;
    this.LeagueService.getLeagueMatchups(this.leagueId, this.selectedWeek).pipe(take(1)).subscribe({
      next: rawPairs => {
        this.matchupsGrouped = rawPairs.map(pair => {
          const teamAInfo = this.standings.find(t => t.roster.roster_id === pair.teamA.roster_id);
          const teamBInfo = this.standings.find(t => t.roster.roster_id === pair.teamB.roster_id);

          // Only highlight if week is in the past
          const highlightA = this.selectedWeek < this.LeagueService.getNflState().week
            ? (pair.teamA.points > pair.teamB.points ? 'win' : 'loss') : '';
          const highlightB = this.selectedWeek < this.LeagueService.getNflState().week
            ? (pair.teamB.points > pair.teamA.points ? 'win' : 'loss') : '';
          // Set Match Status
